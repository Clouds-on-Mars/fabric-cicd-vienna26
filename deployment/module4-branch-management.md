...
- ✅ Completed [Module 3: Version Control](../versioning/module3-version-control.md)
...
1. **Verify your dev workspace is still connected to main**
   - Go to `DEWorkshop_<username>` → Workspace Settings → Git integration
   - Should show `main` branch

2. **Verify your prod workspace exists and is connected to production branch**
   - Go to `DEWorkshop_<username>_prod` → Workspace Settings → Git integration
   - Should show `production` branch

3. **If branches are messed up in Azure DevOps**
   - Go to Azure DevOps → Repos → Branches
   - Delete and recreate the `production` branch from `main`

4. **If workspace is broken, recreate it**
   - Delete the broken workspace in Fabric
   - Use "Branch out" again from your dev workspace

Ask an instructor if you're unsure which state you're in.
...
- ✅ Used Fabric's "Branch out" feature to create a production workspace
- ✅ Understood how branches map to workspaces
- ✅ Discovered that code syncs via Git but data connections don't
- ✅ Learned why deployment automation is essential for enterprise use
- ✅ Prepared for proper deployment pipeline setup in the next module
...
| [← Module 3: Version Control](../versioning/module3-version-control.md) | [Home](../README.md) | [Module 5: Deployment Pipelines →](./module5-deployment-pipelines.md) |
|:-----------------------------------------------------|:--------------------:|-----------------------------------------------:|
