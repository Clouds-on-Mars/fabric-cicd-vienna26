...
- Completed bootstrap deployment: [First Deployment](./module2-first-deployment.md)
- Completed branch development: [Branch Out to New Workspace](./module4-branch-management.md)
...
1. **Check variable library syntax**
   - Open `MyVarLib` in your workspace
   - Verify all variable names match exactly (case-sensitive)
   - Ensure IDs are correct GUIDs (no extra spaces)

2. **Verify workspace assignments**
   - Open Deployment Pipeline settings
   - Confirm Dev and Prod stages are assigned to correct workspaces

3. **If deployment rules aren't applying**
   - Check that target items exist in both workspaces
   - Verify rule syntax matches item types

4. **If shortcuts are broken after deployment**
   - Open Lakehouse_Silver in Production
   - Click "Update all variables" in the ribbon
   - Verify the active value set is correct

5. **Start fresh if totally broken**
   - Delete the deployment pipeline and recreate it
   - Reimport the variable library template

Ask an instructor if you need help debugging variable or rule configurations.
...
- ✅ Created a deployment pipeline with Dev and Prod stages
- ✅ Configured a variable library with environment-specific values
- ✅ Set up deployment rules to remap connections automatically
- ✅ Ran a deployment and validated the production workspace
- ✅ Understood why data isolation requires deployment automation
...
| [← Module 4: Branch Management](./module4-branch-management.md) | [Home](../README.md) | [Module 6: End-to-End Pipeline →](./full-run.md) |
|:-------------------------------------------------|:--------------------:|-------------------------------------------------:|
